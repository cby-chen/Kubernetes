...
service：10.96.0.0/16  <br>
pod：172.16.0.0/16 <br>
...
export IP="192.168.1.198 192.168.1.200 192.168.1.201 192.168.1.202 192.168.1.203"
...
ssh root@192.168.1.198 "rm -rf /etc/machine-id; systemd-machine-id-setup;"
ssh root@192.168.1.200 "rm -rf /etc/machine-id; systemd-machine-id-setup;"
ssh root@192.168.1.201 "rm -rf /etc/machine-id; systemd-machine-id-setup;"
ssh root@192.168.1.202 "rm -rf /etc/machine-id; systemd-machine-id-setup;"
ssh root@192.168.1.203 "rm -rf /etc/machine-id; systemd-machine-id-setup;"
# 
# 查看当前的网卡列表和 UUID：
# nmcli con show
# 删除要更改 UUID 的网络连接：
...
ssh root@192.168.1.198 "nmcli con show"

# 更改网卡的UUID
# 先配置静态IP之后使用ssh方式配置不断连
ssh root@192.168.1.198 "nmcli con delete uuid 00dbed84-fc7c-38b3-bbf7-31280d836a99;nmcli con add type ethernet ifname ens32 con-name ens32;nmcli con up ens32"
ssh root@192.168.1.200 "nmcli con delete uuid 00dbed84-fc7c-38b3-bbf7-31280d836a99;nmcli con add type ethernet ifname ens32 con-name ens32;nmcli con up ens32"
ssh root@192.168.1.201 "nmcli con delete uuid 00dbed84-fc7c-38b3-bbf7-31280d836a99;nmcli con add type ethernet ifname ens32 con-name ens32;nmcli con up ens32"
ssh root@192.168.1.202 "nmcli con delete uuid 00dbed84-fc7c-38b3-bbf7-31280d836a99;nmcli con add type ethernet ifname ens32 con-name ens32;nmcli con up ens32"
ssh root@192.168.1.203 "nmcli con delete uuid 00dbed84-fc7c-38b3-bbf7-31280d836a99;nmcli con add type ethernet ifname ens32 con-name ens32;nmcli con up ens32"
...
# 
# nmcli con add type ethernet ifname ens32 con-name ens32
# 添加一种以太网连接类型，并指定接口名为 ens32，连接名称也为 ens32。
# 
# nmcli con up ens32
# 开启 ens32 这个网络连接。
# 
# 简单来说，这个命令的作用是删除一个特定的网络连接配置，并添加一个名为 ens32 的以太网连接，然后启用这个新的连接。

# 修改静态的IPv4地址
ssh root@192.168.1.198 "nmcli con mod ens32 ipv4.addresses 192.168.1.31/24; nmcli con mod ens32 ipv4.gateway  192.168.1.1; nmcli con mod ens32 ipv4.method manual; nmcli con mod ens32 ipv4.dns "223.5.5.5"; nmcli con up ens32"
ssh root@192.168.1.200 "nmcli con mod ens32 ipv4.addresses 192.168.1.32/24; nmcli con mod ens32 ipv4.gateway  192.168.1.1; nmcli con mod ens32 ipv4.method manual; nmcli con mod ens32 ipv4.dns "223.5.5.5"; nmcli con up ens32"
ssh root@192.168.1.201 "nmcli con mod ens32 ipv4.addresses 192.168.1.33/24; nmcli con mod ens32 ipv4.gateway  192.168.1.1; nmcli con mod ens32 ipv4.method manual; nmcli con mod ens32 ipv4.dns "223.5.5.5"; nmcli con up ens32"
ssh root@192.168.1.202 "nmcli con mod ens32 ipv4.addresses 192.168.1.34/24; nmcli con mod ens32 ipv4.gateway  192.168.1.1; nmcli con mod ens32 ipv4.method manual; nmcli con mod ens32 ipv4.dns "223.5.5.5"; nmcli con up ens32"
ssh root@192.168.1.203 "nmcli con mod ens32 ipv4.addresses 192.168.1.35/24; nmcli con mod ens32 ipv4.gateway  192.168.1.1; nmcli con mod ens32 ipv4.method manual; nmcli con mod ens32 ipv4.dns "223.5.5.5"; nmcli con up ens32"
...
# 
# "nmcli con mod ens32 ipv4.addresses 192.168.1.31/24"
# 修改ens32网络连接的IPv4地址为192.168.1.31，子网掩码为 24。
# 
# "nmcli con mod ens32 ipv4.gateway 192.168.1.1"
# 修改ens32网络连接的IPv4网关为192.168.1.1。
# 
# "nmcli con mod ens32 ipv4.method manual"
# 将ens32网络连接的IPv4配置方法设置为手动。
# 
# "nmcli con mod ens32 ipv4.dns "223.5.5.5"
# 将ens32网络连接的IPv4 DNS服务器设置为 223.5.5.5。
# 
# "nmcli con up ens32"
# 启动ens32网络连接。
# 
# 总体来说，这条命令是通过SSH远程登录到指定的主机，并使用网络管理命令 (nmcli) 修改ens32网络连接的配置，包括IP地址、网关、配置方法和DNS服务器，并启动该网络连接。

# 我这里有公网的IPv6的地址，但是是DHCP动态的，无法固定，使用不方便
# 所以我配置了内网的IPv6地址，可以实现固定的访问地址

# 我使用的方式。只配置IPv6地址不配置网关DNS
ssh root@192.168.1.31 "nmcli con mod ens32 ipv6.addresses fc00::31/8; nmcli con up ens32"
ssh root@192.168.1.32 "nmcli con mod ens32 ipv6.addresses fc00::32/8; nmcli con up ens32"
ssh root@192.168.1.33 "nmcli con mod ens32 ipv6.addresses fc00::33/8; nmcli con up ens32"
ssh root@192.168.1.34 "nmcli con mod ens32 ipv6.addresses fc00::34/8; nmcli con up ens32"
ssh root@192.168.1.35 "nmcli con mod ens32 ipv6.addresses fc00::35/8; nmcli con up ens32"

# IPv6地址路由DNS，样例
ssh root@192.168.1.31 "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::10; nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod ens32 ipv6.method manual; nmcli con mod ens32 ipv6.dns "2400:3200::1"; nmcli con up ens32"
ssh root@192.168.1.32 "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::20; nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod ens32 ipv6.method manual; nmcli con mod ens32 ipv6.dns "2400:3200::1"; nmcli con up ens32"
ssh root@192.168.1.33 "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::30; nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod ens32 ipv6.method manual; nmcli con mod ens32 ipv6.dns "2400:3200::1"; nmcli con up ens32"
ssh root@192.168.1.34 "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::40; nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod ens32 ipv6.method manual; nmcli con mod ens32 ipv6.dns "2400:3200::1"; nmcli con up ens32"
ssh root@192.168.1.35 "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::50; nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1; nmcli con mod ens32 ipv6.method manual; nmcli con mod ens32 ipv6.dns "2400:3200::1"; nmcli con up ens32"
...
# 
# "nmcli con mod ens32 ipv6.addresses fc00:43f4:1eea:1::10"
# 使用nmcli命令修改ens32接口的IPv6地址为fc00:43f4:1eea:1::10。
# 
# "nmcli con mod ens32 ipv6.gateway fc00:43f4:1eea:1::1"
# 使用nmcli命令修改ens32接口的IPv6网关为fc00:43f4:1eea:1::1。
# 
# "nmcli con mod ens32 ipv6.method manual"
# 使用nmcli命令将ens32接口的IPv6配置方法修改为手动配置。
# 
# "nmcli con mod ens32 ipv6.dns "2400:3200::1"
# 使用nmcli命令设置ens32接口的IPv6 DNS服务器为2400:3200::1。
# 
# "nmcli con up ens32"
# 使用nmcli命令启动ens32接口。
# 
# 这个命令的目的是在远程主机上配置ens32接口的IPv6地址、网关、配置方法和DNS服务器，并启动ens32接口。

# 查看网卡配置
# nmcli device show ens32
# nmcli con show ens32
[root@localhost ~]#  cat /etc/NetworkManager/system-connections/ens32.nmconnection 
[connection]
id=ens32
uuid=62bddb11-f254-492a-bf4b-e628e5e04c35
type=ethernet
interface-name=ens32
timestamp=1778382884
...
- `vrrp_instance`部分定义了一个VRRP实例。`VI_1`是实例的名称。
    ...
    - `interface`参数指定了要监听的网卡，这里设置为ens32。
...
- `--service-cluster-ip-range=10.96.0.0/16,fd00:1111::/112` 指定服务CIDR范围。
...
--service-cluster-ip-range：定义 Service 的 IP 范围，这里设置为 10.96.0.0/16 和 fd00::/108。
--cluster-cidr：定义集群的 CIDR 范围，这里设置为 172.16.0.0/16 和 fc00::/48。
...
#    e. qps: 5
#       - 每秒向API服务器发送的请求数量。
# 4. clusterCIDR: 172.16.0.0/16,fd00:2222::/112
